...
const index = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        const res = await axios.get(`https://internshala-clone-6qrv.onrender.com/api/job/${id}`);
        setjob(res.data);
      } catch (error) {
        console.log(error);
      }
  ...
  if (!jobData) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="animation-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div>
      </div>
    );
  }
  ...
  const handlesubmitapplication = async () => {
    if (!coverLetter.trim()) {
      toast.error("Please wrtie a cover letter");
      return;
    }
    if (!availability) {
    ...
    try {
      ...
      await axios.post(
        "https://internshala-clone-6qrv.onrender.com/api/application",
        applicationdata,
      );
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-6 border-b">
          ...
          <p className="text-gray-600 mb-6">{jobData.numberofopening}</p>
        </div>
        {/* Apply Button */}
      ...
      {isModelOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4 mzx-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 px-8 space-y-6">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  Your current resume will be submitted with application
                </p>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
